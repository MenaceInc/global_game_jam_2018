...
void update_brain_alien(Entity *e, Map *m, LightState lighting[MAX_EXPLORER]) {
    ...
    do_light(lighting + EXPLORER_IR, e->x + e->w/2, e->y + e->h/2, 480, 4, 1, 0, 0);
    do_light(lighting + EXPLORER_EM, e->x + e->w/2, e->y + e->h/2, 640, 6, 0.7, 0, 1);
    ...
    if(b->target_entity < 0) {
        ...
        if(ent->type >= 0 && ent->id >= 0 && distance2_32(e->x, e->y, ent->x, ent->y) < 256*256) {
